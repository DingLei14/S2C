...
class LoRA_Dino(nn.Module):
    def __init__(self, dino_model, r: int, lora_layer=[8,9,10,11]): #lora_layer=None
        super(LoRA_Dino, self).__init__()        
        assert r > 0
        # base_vit_dim = dino_model.image_encoder.patch_embed.proj.out_channels
        # dim = base_vit_dim
        if lora_layer:
            self.lora_layer = lora_layer
        else:
            self.lora_layer = list(range(len(dino_model.image_encoder.blocks)))
...
class Dino_CD_LoRA(nn.Module):
    def __init__(self, num_embed=16):
        super(Dino_CD_LoRA, self).__init__()
        dino =  DinoV2_self(model_name='dinov2_vitb14', layer1=11, facet1="value", use_cls=False, norm_descs=True, device="cuda", pretrained=True)
        self.dino = LoRA_Dino(dino, r=4)
        self.Adapter = nn.Sequential(nn.Conv2d(768, 128, kernel_size=1, stride=1, padding=0, bias=False),
                                     nn.BatchNorm2d(128), nn.ReLU(), nn.Conv2d(128, num_embed, kernel_size=1))
    ...
    def run_pretrain(self, image):
        image_embeddings = self.dino.get_image_embeddings(image)
    ...
    def forward(self, x: torch.Tensor):
        if x.shape[-1]%14:
            new_size = 14*(x.shape[-1]//14+1)
            x = F.interpolate(x, [new_size, new_size], mode="bilinear")        
    ...
    def bi_forward(self, x1: torch.Tensor, x2: torch.Tensor):
        ...
        return F.interpolate(yc, input_shape, mode="bilinear")